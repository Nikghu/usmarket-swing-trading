...
|---|---|---|---|---|---|---|---|
| SRD-ANA-001.001 | FO-ANA-001 | Must | `LiveEngine.subscribe(symbols)` calls `IBKRClient.subscribe_realtime_bars(symbol, bar_size=5)` for each symbol in the watchlist (max 20). Unsubscribes any symbol no longer in the list. | `list[str]` symbols, `IBKRClient` | active IBKR 5s bar subscriptions | Subscription count must never exceed 20; excess symbols are queued with warning | Draft |
| SRD-ANA-001.002 | FO-ANA-001 | Must | `CandleBuilder` maintains one rolling buffer per (symbol, timeframe). On each incoming 5s bar: append to buffer; check if the current candle duration has elapsed; if yes, finalise the candle. Supported timeframes: 1m, 3m, 5m, 15m, 1h. | `RealtimeBar(symbol, time, open, high, low, close, volume)` | `OHLCVBar` emitted on candle close | Buffer is in-memory only; lost on restart (live candles restart fresh from next bar) | Draft |
| SRD-ANA-001.003 | FO-ANA-001 | Must | Candle aggregation rules: `open` = first-bar open, `high` = max(high), `low` = min(low), `close` = last-bar close, `volume` = sum(volume). Applied identically for all timeframes. | list of 5s bars within window | `OHLCVBar` | Same function used for both historical synthesis and live construction to guarantee consistency | Draft |
...
| SRD-ANA-001.005 | FO-ANA-001 | Must | A gap in 5s bar delivery (no bar received in a 5s window, e.g., no trades) is handled by carrying forward the previous bar's close as O/H/L/C and setting volume = 0. Applies only during market hours. | missing 5s bar | synthetic gap bar; aggregation continues | Must not produce negative volume or NaN OHLC values | Draft |
